...
public class MoviePlayer {
    ...
    public MoviePlayer(File sourceFile, Surface outputSurface, FrameCallback frameCallback)
        ...
        try {
            ...
            if (trackIndex < 0) {
                throw new RuntimeException(Strings.NO_VIDEO_TRACK_FOUND_IN + mSourceFile);
            }
            ...
            if (VERBOSE) {
                Logger.d(Strings.VIDEO_SIZE_IS + mVideoWidth + "x" + mVideoHeight);
            }
        } finally {
            if (extractor != null) {
                extractor.release();
    ...
    private static int selectTrack(MediaExtractor extractor) {
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (mime.startsWith("video/")) {
                if (VERBOSE) {
                    Logger.d(Strings.EXTRACTOR_SELECTED_TRACK + i + " (" + mime + "): " + format);
                }
                return i;
            }
        }
    ...
    private void doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
        ...
        while (!outputDone) {
            if (VERBOSE) Logger.d(Strings.LOOP);
            if (mIsStopRequested) {
                Logger.d(Strings.STOP_REQUESTED);
                return;
            }
            ...
            if (!inputDone) {
                ...
                if (inputBufIndex >= 0) {
                    ...
                    if (chunkSize < 0) {
                        ...
                        inputDone = true;
                        if (VERBOSE) Logger.d(Strings.SENT_INPUT_EOS);
                    } else {
                        if (extractor.getSampleTrackIndex() != trackIndex) {
                            Logger.w("WEIRD: got sample from track " +
                        ...
                        if (VERBOSE) {
                            Logger.d( "submitted frame " + inputChunk + " to dec, size=" +
                                    chunkSize);
                        }
                        inputChunk++;
                        extractor.advance();
                    ...
                    if (VERBOSE) Logger.d(Strings.INPUT_BUFFER_NOT_AVAILABLE);
                }
            }
            ...
            if (!outputDone) {
                ...
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (VERBOSE) Logger.d(Strings.NO_OUTPUT_FROM_DECODER_AVAILABLE);
                } else if (decoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    // not important for us, since we're using Surface
                    if (VERBOSE) Logger.d(Strings.DECODER_OUTPUT_BUFFERS_CHANGED);
                } else if (decoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    MediaFormat newFormat = decoder.getOutputFormat();
                    if (VERBOSE) Logger.d("decoder output format changed: " + newFormat);
                    ...
                    if (firstInputTimeNsec != 0) {
                        ...
                        Logger.d("startup lag " + ((nowNsec-firstInputTimeNsec) / 1000000.0) + " ms");
                        firstInputTimeNsec = 0;
                    }
                    boolean doLoop = false;
                    if (VERBOSE) Logger.d("surface decoder given buffer " + decoderStatus +
                            " (size=" + mBufferInfo.size + ")");
                    if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        if (VERBOSE) Logger.d(Strings.OUTPUT_EOS);
                        if (mLoop) {
                            doLoop = true;
                        } else {
                            outputDone = true;
                    ...
                    if (doLoop) {
                        Logger.d(Strings.REACHED_EOS_LOOPING);
